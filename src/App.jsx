...
// ==================== 首页 ====================
function HomePage({ onNavigate, onShowQR }) {
  const homeStats = [
  ...
  return (
    <>
      {/* Hero */}
      <header style={{ position: "relative", zIndex: 10, maxWidth: 1280, margin: "0 auto", padding: "160px 24px 100px", textAlign: "center" }}>
        <div style={{ display: "inline-flex", alignItems: "center", gap: 8, padding: "8px 20px", fontSize: 13, fontWeight: 600, color: "#00f5ff", background: "rgba(0,245,255,0.1)", border: "1px solid rgba(0,245,255,0.2)", borderRadius: 999, marginBottom: 32, letterSpacing: "0.1em", textTransform: "uppercase" }}>
          <span style={{ width: 6, height: 6, borderRadius: "50%", background: "#00f5ff", boxShadow: "0 0 10px #00f5ff" }} />
          AI Coding Training
        </div>
        <h1 style={{ fontSize: 64, fontWeight: 800, lineHeight: 1.15, marginBottom: 24, letterSpacing: "-0.02em" }}>
        ...
        <div style={{ display: "flex", gap: 16, justifyContent: "center", flexWrap: "wrap" }}>
          <button onClick={() => onShowQR("trial")} style={{ padding: "16px 36px", fontSize: 16, fontWeight: 600, color: "#050510", background: "linear-gradient(135deg, #00f5ff, #00d4ff)", borderRadius: 12, border: "none", cursor: "pointer", transition: "all 0.3s ease", boxShadow: "0 0 40px rgba(0,245,255,0.4), 0 8px 32px rgba(0,0,0,0.3)" }}
            onMouseEnter={(e) => { e.target.style.transform = "translateY(-2px) scale(1.02)"; e.target.style.boxShadow = "0 0 60px rgba(0,245,255,0.5), 0 12px 40px rgba(0,0,0,0.4)"; }}
            onMouseLeave={(e) => { e.target.style.transform = "translateY(0) scale(1)"; e.target.style.boxShadow = "0 0 40px rgba(0,245,255,0.4), 0 8px 32px rgba(0,0,0,0.3)"; }}
          >试听课程</button>
          <button onClick={() => onNavigate("courses")} style={{ padding: "16px 36px", fontSize: 16, fontWeight: 500, color: "#ffffff", background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 12, cursor: "pointer", transition: "all 0.3s ease" }}
            onMouseEnter={(e) => { e.target.style.background = "rgba(255,255,255,0.1)"; e.target.style.borderColor = "rgba(0,245,255,0.3)"; e.target.style.transform = "translateY(-2px)"; }}
            onMouseLeave={(e) => { e.target.style.background = "rgba(255,255,255,0.05)"; e.target.style.borderColor = "rgba(255,255,255,0.15)"; e.target.style.transform = "translateY(0)"; }}
          >查看课程体系</button>
        </div>
      ...
      {/* 统计 — 使用统一统计数据 */}
      <section style={{ position: "relative", zIndex: 10, maxWidth: 1280, margin: "0 auto", padding: "80px 24px" }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 24 }}>
          {homeStats.map((s) => (
            <GlassCard key={s.label} hoverable glowColor={s.color === "#00f5ff" ? "cyan" : s.color === "#8b5cf6" ? "purple" : "green"} style={{ textAlign: "center", padding: "32px 24px" }}>
              <div style={{ width: 48, height: 48, margin: "0 auto 16px", borderRadius: 12, background: s.color + "15", border: "1px solid " + s.color + "30", display: "flex", alignItems: "center", justifyContent: "center", color: s.color }}><TechIcon type={s.icon} /></div>
              <div style={{ fontSize: 40, fontWeight: 800, color: s.color, lineHeight: 1.1, marginBottom: 8, fontFamily: "var(--font-mono)" }}>{s.num}</div>
              <div style={{ fontSize: 14, color: "rgba(255,255,255,0.6)", fontWeight: 500 }}>{s.label}</div>
            </GlassCard>
          ))}
        </div>
      </section>
      ...
      {/* CTA */}
      <section style={{ position: "relative", zIndex: 10, maxWidth: 800, margin: "0 auto", padding: "80px 24px", textAlign: "center" }}>
        <GlassCard hoverable={false} style={{ padding: "60px 48px", background: "linear-gradient(135deg, rgba(0,245,255,0.05), rgba(139,92,246,0.05))", border: "1px solid rgba(0,245,255,0.2)" }}>
          <h2 style={{ fontSize: 32, fontWeight: 700, marginBottom: 16, color: "#ffffff" }}>开启你的 AI 编程之旅</h2>
          <p style={{ fontSize: 16, color: "rgba(255,255,255,0.6)", marginBottom: 32, lineHeight: 1.7 }}>加入{config.company.shortName}，与全国优秀学员一起，掌握前沿 AI 技术，赢在职业起跑线。</p>
          <button onClick={() => onShowQR("consult")} style={{ padding: "16px 48px", fontSize: 16, fontWeight: 600, color: "#050510", background: "linear-gradient(135deg, #00f5ff, #8b5cf6)", borderRadius: 12, border: "none", cursor: "pointer", transition: "all 0.3s ease", boxShadow: "0 0 40px rgba(0,245,255,0.3), 0 8px 32px rgba(0,0,0,0.3)" }}
            onMouseEnter={(e) => { e.target.style.transform = "translateY(-2px) scale(1.02)"; e.target.style.boxShadow = "0 0 60px rgba(0,245,255,0.4), 0 12px 40px rgba(0,0,0,0.4)"; }}
            onMouseLeave={(e) => { e.target.style.transform = "translateY(0) scale(1)"; e.target.style.boxShadow = "0 0 40px rgba(0,245,255,0.3), 0 8px 32px rgba(0,0,0,0.3)"; }}
          >立即咨询</button>
        </GlassCard>
      </section>
    </>
    ...
    <div style={{ minHeight: "100vh", position: "relative" }}>
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: scrolled ? "rgba(5,5,16,0.95)" : "rgba(5,5,16,0.6)", backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)", borderBottom: scrolled ? "1px solid rgba(255,255,255,0.08)" : "1px solid transparent", transition: "all 0.3s ease" }}>
        <div style={{ maxWidth: 1280, margin: "0 auto", padding: "0 24px", height: 72, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12, cursor: "pointer" }} onClick={() => navigateTo("home")}>
            <img src={logoImg} alt={config.company.shortName} style={{ height: 40, width: "auto", display: "block", objectFit: "contain" }} />
            <span style={{ fontSize: 20, fontWeight: 700, color: "#ffffff", letterSpacing: "0.05em" }}>{config.company.shortName}</span>
          </div>
      ...
      {/* Page Content */}
      {currentPage === "home" && <HomePage onNavigate={navigateTo} onShowQR={setQrModalVariant} />}
      {currentPage === "courses" && <CoursePage onShowQR={setQrModalVariant} />}
      {currentPage === "students" && <StudentPage onShowSalary={setSalaryModal} onShowQR={setQrModalVariant} />}
      {currentPage === "services" && <ServicePage onShowQR={setQrModalVariant} onNavigate={navigateTo} />}
      {currentPage === "about" && <AboutPage onShowQR={setQrModalVariant} />}
      ...
      {/* Footer — 使用统一配置 */}
      <footer style={{ position: "relative", zIndex: 10, textAlign: "center", padding: "40px 24px", fontSize: 13, color: "rgba(255,255,255,0.4)", borderTop: "1px solid rgba(255,255,255,0.05)" }}>
